...
const getEvaluacionesEvaInt = async (req, res) => {
  try {
    const { id } = req.params;
...
const getEvaluacionesModal = async (req, res) => {
  try {
    const { id } = req.params;
    // const { rol } = req.body;
    // console.log(rol);
...
const getTTextosRequisito = async (req, res) => {
  try {
    const { id } = req.params;
...
const calificarEvaInt = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    if (estado_so == 0) {
      ...
      const observaciones = "Calificación Interna de la solicitud Aceptada";
      await connection
        .promise()
        .query(
...
const actualizarSolicitudesEvaInt = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    const sqlQuery = `
      UPDATE pmo_administracion_solicitud_obra
      SET
          SOL_ESTADO = CASE
                          WHEN (SELECT COUNT(*) FROM pmo_administracion_evaluacion_obra WHERE DOC_ID = ? AND EVA_ESTADO = 2 AND EVA_ESTADO_LOGICO = 1 ) = 2 THEN 3
                          WHEN (SELECT COUNT(*) FROM pmo_administracion_evaluacion_obra WHERE DOC_ID = ? AND EVA_ESTADO = 1 AND EVA_ESTADO_LOGICO = 1) > 0 AND (SELECT COUNT(*) FROM pmo_administracion_evaluacion_obra WHERE DOC_ID = ? AND EVA_ESTADO = 2 AND EVA_ESTADO_LOGICO = 1) > 0 THEN SOL_ESTADO
                          WHEN (SELECT COUNT(*) FROM pmo_administracion_evaluacion_obra WHERE DOC_ID = ? AND EVA_ESTADO = 1 AND EVA_ESTADO_LOGICO = 1) > 0 THEN 2
                          ELSE SOL_ESTADO
                      END,
          SOL_ESTADO_NOMBRE = CASE
                                 WHEN (SELECT COUNT(*) FROM pmo_administracion_evaluacion_obra WHERE DOC_ID = ? AND EVA_ESTADO = 2 AND EVA_ESTADO_LOGICO = 1) = 2 THEN 'Rechazado'
                                 WHEN (SELECT COUNT(*) FROM pmo_administracion_evaluacion_obra WHERE DOC_ID = ? AND EVA_ESTADO = 1 AND EVA_ESTADO_LOGICO = 1) > 0 AND (SELECT COUNT(*) FROM pmo_administracion_evaluacion_obra WHERE DOC_ID = ? AND EVA_ESTADO = 2 AND EVA_ESTADO_LOGICO = 1) > 0 THEN SOL_ESTADO_NOMBRE
                                 WHEN (SELECT COUNT(*) FROM pmo_administracion_evaluacion_obra WHERE DOC_ID = ? AND EVA_ESTADO = 1 AND EVA_ESTADO_LOGICO = 1) > 0 THEN 'Aceptado'
                                 ELSE SOL_ESTADO_NOMBRE
                             END
      WHERE DOC_ID = ?;
`;
    ...
    const result = await connection
      .promise()
      .query(sqlQuery, [id, id, id, id, id, id, id, id, id]);
    res.json(result);
  } catch (error) {
    res.status(500);
    res.send(error.message);
...
const crearInformeInt = async (req, res) => {
  try {
    const {
      infoEvaluador,
      infoTipo,
...
const getUsuariosFiltradoInt = async (req, res) => {
  try {
    const { id } = req.params;
    // const { rol } = req.body;
    // console.log(rol);
...
const guardarPdfInt = async (req, res) => {
  try {
    ...
    upload(req, res, async (err) => {
      ...
      return res.json({
        message: "Archivo PDF recibido y guardado exitosamente.",
      });
    });
  } catch (error) {
...
const cambiarEstadoSolEvaInt = async (req, res) => {
  try {
    const { id } = req.params;
...
export const methods = {
  cambiarEstadoSolEvaInt,
  guardarPdfInt,
  actualizarSolicitudesEvaInt,
  calificarEvaInt,
  getTTextosRequisito,
  getEvaluacionesModal,
  getEvaluacionesEvaInt,
  updateContrasena,
  crearInformeInt,
  getUsuariosFiltradoInt,
};
