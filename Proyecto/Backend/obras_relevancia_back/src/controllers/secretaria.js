...
const getEvaluacionesSecretaria = async (req, res) => {
  try {
    const { id } = req.params;
    // const { rol } = req.body;
    // console.log(rol);
...
const getEvaluadoresSecretaria = async (req, res) => {
  try {
    const connection = await getConnection();
    const [result] = await connection
      .promise()
...
const getUsuariosSecretaria = async (req, res) => {
  try {
    const connection = await getConnection();
    const [result] = await connection
      .promise()
...
const getUsuariosSecretariaFiltrado = async (req, res) => {
  try {
    const { id } = req.params;
    // const { rol } = req.body;
    // console.log(rol);
...
const updateSolInicial = async (req, res) => {
  try {
    ...
    const observaciones =
      "Estado de la solcitud actualizada a: " + Estado + ". " + observacionesModal;
    await connection
      .promise()
      .query(
...
const getLinkEvaluaciones = async (req, res) => {
  try {
    ...
    const result = await connection
      ...
      .query(
        "SELECT * FROM pmo_administracion_informe WHERE EVA_ID = ?;",
        [id]
      );
    res.json(result[0]);
  } catch (error) {
...
export const methods = {
  getLinkEvaluaciones,
  updateSolInicial,
  getUsuariosSecretariaFiltrado,
  getUsuariosSecretaria,
  borrarEvaluador,
  asignarEvaluadorObra,
  getEvaluadoresSecretaria,
  getEvaluacionesSecretaria,
  getSolicitudesModal,
  getSolicitudesTabla,
  getUsuariosWithRol,
  getEvaluadores,
