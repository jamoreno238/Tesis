...
import { getEvaluadoresSecretaria } from "../../../services/secretariaServicios";
import { asignarEvaluadorSolicitud } from "../../../services/secretariaServicios";
...
const AsignarEvaluador = ({ isOpen, onClose, tableDataEva }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await getEvaluadoresSecretaria();
  ...
  const handleAsignarClick = (codigo, eva_tipo, eva_evaluador) => {
    ...
    const calificacionEvaluadores = tableDataEva.filter(
      (row) =>
        row.EVA_ESTADO_NOMBRE === "Aceptado" ||
        row.EVA_ESTADO_NOMBRE === "Rechazado"
    );

    if (calificacionEvaluadores.length === 2) {
      // Verificar si los dos estados son diferentes
      if (
        calificacionEvaluadores[0].EVA_ESTADO_NOMBRE !==
          calificacionEvaluadores[1].EVA_ESTADO_NOMBRE &&
        tableDataEva.length < 3
      ) {
        // const datosTabla = { doc_id, evaTipoCalificadorTercero, eva_evaluador };
        const datosTabla = {
          doc_id: doc_id,
          eva_tipo: "Evaluación Tercera",
          eva_evaluador: eva_evaluador,
        };
    ...
    const evaluadorExistente = tableDataEva.find(
      (row) =>
        row.EVA_TIPO === "Evaluación Interna" ||
        row.EVA_TIPO === "Evaluación Externa"
    );
            ...
            <TableBody>
              {tableData
                .filter(
                  (row) =>
                    tipoFilter === "Todos" ||
                    (row.ROL_ID === 4 && tipoFilter === "Interno") ||
                    (row.ROL_ID === 5 && tipoFilter === "Externo")
                )
                .map((row) => (
