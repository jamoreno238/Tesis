...
import { getSolicitudesTablaModal } from "../../../services/secretariaServicios";
...
const DetalleModal = ({ open, handleClose, selectedRow }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (selectedRow && selectedRow.DOC_ID) {
        ...
        try {
          const result = await getSolicitudesTablaModal(selectedRow.DOC_ID);
      ...
      >
        ...
        <Grid container spacing={2}>
          ...
          <Grid item xs={4}>
            <TextField
              label="Código"
              value={selectedRow && selectedRow.DOC_ID ? selectedRow.DOC_ID : ""}
              fullWidth
              disabled
            />
          </Grid>
