...
import {
  getEvaluacionesEvaluadorInternoModal,
  obtenerRequisitos,
  crearInformeInt,
} from "../../../services/evaluadorInternoServicios";
import { calificacionEvaInt } from "../../../services/evaluadorInternoServicios";
import TextField from "@mui/material/TextField";
import Cookies from "js-cookie";
import { decodeToken } from "react-jwt";
import DialogoCalificar from "../../componentes/evaluadorInterno/DialogoCalificar";
import { useNavigate } from "react-router-dom";
import { getUsuariosFiltrado } from "../../../services/evaluadorInternoServicios.js";
import { guardarReporteInterno } from "../../../services/evaluadorInternoServicios.js";
import { cambiarEstadoSolEvaInt } from "../../../services/evaluadorInternoServicios.js";
...
const EvaluacionesEvaInt = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const result = await getEvaluacionesEvaluadorInternoModal(codigo);
        ...
        const requisitos = await obtenerRequisitos(tipo_req);
  ...
  }, [checkboxStates, tableDataRequisitos]); // New state to manage checkbox states
  ...
  const handleAcceptDelete = async () => {
    ...
    if (allCheckboxesSelected) {
      ...
      // Llama a crearInformeInt con los datos recopilados
      try {
        await crearInformeInt({
          infoEvaluador: idEva,
          infoTipo: tipo_req,
          infoInforme,
          infoRecomendacion,
        ...
        if (token) {
          ...
          calificacionEvaInt(codigo, datos);

          navigate(`/misEvaluacionesEvaInt`);
        }
      } catch (error) {
  ...
  const generarPDF = async (valorRecomendador) => {
    ...
    guardarReporteInterno(datos);
    cambiarEstadoSolEvaInt(idSolicitud);
  };
...
export default EvaluacionesEvaInt;
