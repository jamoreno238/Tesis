...
import Plantilla from "../../componentes/menu/PlantillaSecretaria";
import TextField from "@mui/material/TextField";
import Button from "@mui/material/Button";
import Grid from "@mui/material/Grid";
import { actualizarContrasena } from "../../../services/secretariaServicios";
...
const CambioContraSecretaria = () => {
  ...
  if (token) {
    const decodedToken = decodeToken(token);
    idUsuario = decodedToken && decodedToken.idUsuario ? decodedToken.idUsuario : null;
  }
  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      const response = await actualizarContrasena({ id: idUsuario, contrasenaActual: contrasenaActual, nuevaContrasena: contrasenaNueva });
      ...
      if (response && response.data && response.data.success) {
        ...
        alert("No se pudo actualizar la contraseña. Por favor verifique su contraseña actual.");
      }
    } catch (error) {
...
export default CambioContraSecretaria;
